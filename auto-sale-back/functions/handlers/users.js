...
const {validateEmail, validatePassword, validateCred, validatePhone} = require('../util/validators');
...
exports.signUp = (req, res) => {
    ...
    db.doc(`/users/${user.email}`)
      ...
      .then((doc) => {
        if (doc.exists) {
          return res.status(400).json({ emailTakenError: 'This email is already taken' });
        } else {
          return firebase
            .auth()
...
exports.changeUserInfo = (req, res) => {
  ...
  .then((doc)=>{
    if (!doc.exists) {
      ...
      user = doc.data();
      return db.collection('users').doc(req.body.email).set(Object.assign(user, newUser))
    }
  })
  .then(()=>{
